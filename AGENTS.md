...
- **Claude Code hooks (fourth source — terminal-mode CLI):** the output-channel sources above only see the Claude Code *extension*. When the standalone `claude` CLI runs in the integrated terminal it writes to no output channel, so a separate path handles it: a hook configured **globally** in `~/.claude/settings.json` runs `~/.codecubicle/capture.cjs` (source of truth committed at `.codecubicle/capture.cjs`), which routes each payload to a **per-workspace** file `~/.codecubicle/sessions/<sessionKey(cwd)>.jsonl` (key from the payload's `cwd`). Global config means `claude` lights up the office in *any* project; per-`cwd` routing keeps each VS Code window scoped to its own workspace. `extension.ts` `subscribeToHookCapture` **polls** the file(s) for its open folders — `hookFilePaths()` maps each folder via `sessionKey` (= `src/sessionKey.ts`, unit-tested; `capture.cjs` duplicates it and the two MUST agree). Polling (not a `FileSystemWatcher`) because the files live under `~`, outside the workspace, and polling proved more reliable for external appends anyway; it primes byte offsets at startup (skip backlog, but read the first real append), consumes only up to the last newline (partial mid-append waits), and resets on truncation. Appended text goes through `parseHookChunk` → `parseHookPayload` (in `parser.ts`, **not** `parseLine` — these are JSON objects, not log lines). Mapping (calibrated against a real capture, Claude Code 2.1.x):
...
| `SuperRetroWorld_CharacterPack_Full/` | Commercial OK **with credit**; no redistributing the assets directly | The empty-office "streaker" (`webview/sprites/streaker.png`, the `free_character_nude` sheet — SFW) |

**Attribution is required** for the Super Retro World assets — see [CREDITS.md](CREDITS.md) (which ships in the .vsix). The streaker sheet is 48×80 = 3 frames × 4 directions of 16×20 (RPGMaker row order: down/left/right/up); the renderer animates the side rows for an occasional dash across the empty office (`tickStreaker`/`drawStreaker`). ⚠️ This is a different format from the Modern-tiles worker sheets — its layout is hardcoded in the renderer, not in `sprites.ts`.
